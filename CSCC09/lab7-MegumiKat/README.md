[![Review Assignment Due Date](https://classroom.github.com/assets/deadline-readme-button-22041afd0340ce965d47ae6ef1cefeee28c7c493a6346c4f15d667ab976d596c.svg)](https://classroom.github.com/a/T_U0wrhA)
# Web Security
...
> In the handout below, we configure HTTPS directly using Express. This is not standard practice however it shows you the concept. Another way is to have a [reverse proxy](https://en.wikipedia.org/wiki/Reverse_proxy) that will handle all HTTPS requests in front of the Express backend.
...
> The certificate we have generated is not considered as secured by the browser because it is not signed by a [Certificate Authority](https://en.wikipedia.org/wiki/Certificate_authority) such as [Let's Encrypt](https://letsencrypt.org/). This can only be done once we own a domain name and we are able to deploy our website behind that domain (which we don't here yet). We will see how to make our certificate valid when we talked about deployment. 
...
> This is one way to enable HTTPS using node/express. We will see another way when we talked about deployment. 
