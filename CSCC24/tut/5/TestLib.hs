...
runTestForMachine (Group ts) = mapM_ runTestForMachine ts
...
runTestForHuman (Group ts) = foldM (\b t -> (b &&) <$> runTestForHuman t) True ts
...
testlibMain tests = do
    arg <- fmap (parseArgs tests) getArgs
    case arg of
      Arg Human t -> do
          b <- runTestForHuman t
          unless b exitFailure
      Arg Machine t -> runTestForMachine t
      ArgError -> error "Invalid test number."

data Args = Arg{aMode :: !Mode, aTest :: !Test}
          | ArgError
data Mode = Human | Machine

parseArgs tests args = go Arg{aMode=Human, aTest = Group tests} args
  where
    go a [] = a
    go a@Arg{aMode, aTest} (x : xs)
      | x == "-q" = go a{aMode=Machine} xs
      | Just i <- readMaybe x, Group ts <- aTest, 0 <= i, i < length ts =
            go a{aTest = ts !! i} xs
      | otherwise = ArgError
